            ...
            <div className="py-4 text-center text-slate-400 space-y-4">
              ...
              <div className="mb-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button
                    onClick={() => { handleNavigate('case-notes'); setCommandPaletteOpen(false); }}
                    className="p-2.5 bg-teal-500/10 hover:bg-teal-500/20 text-teal-300 rounded-xl border border-teal-500/30 text-xs font-bold flex items-center gap-2 transition-all text-left"
                  >
                    <Plus className="w-4 h-4 text-teal-400 shrink-0" />
                    <span>New Case Note</span>
                  </button>
                  <button
                    onClick={() => { handleNavigate('abc-analyser'); setCommandPaletteOpen(false); }}
                    className="p-2.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 rounded-xl border border-emerald-500/30 text-xs font-bold flex items-center gap-2 transition-all text-left"
                  >
                    <Plus className="w-4 h-4 text-emerald-400 shrink-0" />
                    <span>Log ABC Event</span>
                  </button>
                  <button
                    onClick={() => { handleNavigate('billing'); setCommandPaletteOpen(false); }}
                    className="p-2.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 rounded-xl border border-amber-500/30 text-xs font-bold flex items-center gap-2 transition-all text-left"
                  >
              ...
              {/* Recent Participants Quick-Switch */}
              {clients.length > 0 && (
                <div className="mb-3">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-2 text-left">
                    Recent Participants
                  ...
                  <div className="flex flex-wrap gap-1.5 text-left">
                    {clients.slice(0, 6).map((c: any) => (
                      <button
                        key={c.id}
                        onClick={() => { setSelectedClientId(c.id); handleNavigate('clients', c.id, 'CLIENT'); }}
                        className="px-2.5 py-1 bg-slate-950 hover:bg-slate-800 text-slate-300 hover:text-white rounded-lg border border-slate-800 text-xs font-semibold transition-all flex items-center gap-1.5"
                      >
