...
export const Header: React.FC = () => {
  ...
  const [notifCategory, setNotifCategory] = useState<'all' | 'incident' | 'billing' | 'compliance' | 'clinical' | 'hr'>('all');
        ...
        <div className="flex items-center gap-2 sm:gap-2.5">
          ...
          <div className="relative">
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-rose-500 text-white text-[9px] font-bold min-w-[16px] h-4 px-1 rounded-full flex items-center justify-center animate-pulse shadow-sm">
            ...
            {showNotifPopover && (
              <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-4 z-50 text-xs space-y-3 animate-in fade-in duration-150">
                <div className="space-y-2 border-b border-slate-800 pb-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      {unreadCount > 0 && (
                        <span className="text-[9px] bg-rose-500/20 text-rose-300 font-mono px-1.5 py-0.5 rounded border border-rose-500/30">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => notifications.filter((n: AppNotification) => n.read).forEach((n: AppNotification) => dismissNotification(n.id))}
                  ...
                  <div className="flex items-center gap-1 flex-wrap">
                    {(['all', 'incident', 'billing', 'compliance', 'clinical', 'hr'] as const).map(cat => (
                      <button
                        key={cat}
                        onClick={() => setNotifCategory(cat)}
                        className={`px-2 py-0.5 rounded-full text-[9px] font-bold transition-all capitalize ${
                          notifCategory === cat
                      ...
                      >
                        {cat === 'all'
                          ...
                          : `${cat} (${notifications.filter((n: AppNotification) => n.type === cat).length})`}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-2 max-h-72 overflow-y-auto">
                  ...
                  ) : notifications.filter((n: AppNotification) => notifCategory === 'all' || n.type === notifCategory).length === 0 ? (
                    <p className="text-center text-slate-500 py-4">No {notifCategory} notifications.</p>
                  ) : (
                    notifications
                      .filter((n: AppNotification) => notifCategory === 'all' || n.type === notifCategory)
