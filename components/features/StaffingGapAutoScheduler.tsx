...
export const StaffingGapAutoScheduler: React.FC<StaffingGapAutoSchedulerProps> = ({
  ...
  const { clients, practitioners, addAuditLog, addNotification } = useManagementStore();
  ...
  // 1. Ingest client requirements & calculate weekly gap analysis
  const gapAnalysis = useMemo(() => {
    ...
    const overcapacityPracs = practitioners.filter((p) => p.activeCaseloadCount >= p.caseloadLimit);
  ...
  // 2. Intelligent Auto-Scheduling Recommendation Engine
  const suggestedShifts = useMemo(() => {
    const suggestions: SuggestedShift[] = [];
    const targetDate = '2026-08-12'; // Primary planning target date in current active cycle
    ...
    gapAnalysis.clientGaps.forEach((gap, idx) => {
      ...
      const candidatePracs = [...practitioners].filter((p) => {
        const availableSlots = p.caseloadLimit - p.activeCaseloadCount;
                  ...
                  <div className="space-y-1 min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-3 text-[11px] text-slate-400 font-mono pt-1">
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        {sug.date} ({sug.startTime} - {sug.endTime}) • {sug.hours} hrs
                      </span>
                    </div>
                  </div>
