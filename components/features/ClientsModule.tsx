...
import { PreSessionBriefModal } from './PreSessionBriefModal';
import { scoreClientProfile } from '@/lib/clientProfileScorer';
import { analyzeBudgetBurn } from '@/lib/budgetAnalytics';
import { computeClientRiskAssessment } from '@/lib/ai-assistant';
import { RiskAssessment, ABCLog, BSPDocument, CaseNote, Incident } from '@/types';
import {
  Users,
  Search,
  Plus,
...
export const ClientsModule: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
            ...
            {filteredClients.map((client: Client) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Profile Completeness & Budget Burn Ribbon */}
                  {(() => {
                    const prof = scoreClientProfile(client);
                    const burn = (client.planStartDate && client.planEndDate) ? analyzeBudgetBurn(client) : null;
                    return (
                      <div className="flex items-center justify-between gap-1 mt-2 pt-1.5 border-t border-slate-800/60 flex-wrap">
                        <div className="flex items-center gap-1 flex-wrap">
                          <span
                            title={prof.missingFields.length > 0 ? `Missing: ${prof.missingFields.join(', ')}` : 'Profile complete'}
                            className={`text-[9px] font-bold px-1.5 py-0.5 rounded border cursor-help ${
                              prof.tier === 'complete'
                          ...
                          {burn && burn.burnRateStatus !== 'on-track' && (
                            <span
                              title={`Projected end spend: $${burn.projectedEndSpend.toLocaleString()}`}
                              className={`text-[9px] font-bold px-1.5 py-0.5 rounded border cursor-help ${
                                burn.burnRateStatus === 'critical'
                                  ? 'bg-rose-500/20 text-rose-400 border-rose-500/30'
                                  : 'bg-amber-500/10 text-amber-400 border-amber-500/20'
                              }`}
                            >
                              {burn.burnRateStatus === 'critical' ? '⚠ Critical Spend' : '⚠ Budget Risk'}
                            </span>
                          )}
                        </div>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setBriefClientId(client.id);
                          }}
                          className="px-2 py-0.5 bg-teal-500/10 hover:bg-teal-500/20 text-teal-400 text-[10px] font-bold rounded border border-teal-500/20 flex items-center gap-1 transition-all ml-auto"
                          title="Generate AI Pre-Session Clinical Brief"
                        >
      ...
      {/* Pre-Session Clinical Brief Modal (Phase 6.1) */}
      {briefClientId && (() => {
        const briefClient = clients.find((c: Client) => c.id === briefClientId);
        if (!briefClient) return null;
        return (
          <PreSessionBriefModal
            client={briefClient}
            recentNotes={(caseNotes as CaseNote[]).filter((n: CaseNote) => n.clientId === briefClientId).slice(0, 10)}
            recentABCLogs={(abcLogs as ABCLog[]).filter((a: ABCLog) => a.clientId === briefClientId).slice(0, 20)}
            recentIncidents={(incidents as Incident[]).filter((i: Incident) => (i as any).clientId === briefClientId)}
            bspDocuments={(bspDocuments as BSPDocument[]).filter((b: BSPDocument) => b.clientId === briefClientId)}
            onClose={() => setBriefClientId(null)}
          />
        );
      })()}
    </div>
