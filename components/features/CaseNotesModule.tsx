...
export const CaseNotesModule: React.FC = () => {
  ...
  const [goalGasScores, setGoalGasScores] = useState<Record<string, number>>({});
...
${rawText}`;
  ...
  const QUICK_PHRASES: Record<string, string[]> = {
    subjective: [
      'Participant presented engaged and motivated for today\'s session.',
      'Participant arrived calm with reported positive mood baseline.',
      'Participant presented with elevated stress indicators at session commencement.',
      'Carer reported a challenging week with increased frequency of target behaviours.',
    ],
    objective: [
      'Delivered targeted PBS replacement skill reinforcement activities.',
      'Facilitated sensory regulation and co-regulation exercises.',
      'Practiced functional communication training (FCT) role-play scenarios.',
      'Administered structured visual schedule task sequencing routines.',
    ],
    assessment: [
      'Measurable progress observed toward primary NDIS capacity building goals.',
      'Participant demonstrated emerging skill consolidation with support scaffolding.',
      'Engagement consistent with previous baseline — no significant regression noted.',
      'Elevated antecedent exposure noted — recommend proactive strategy review.',
    ],
    plan: [
      'Continue weekly allied health PBS sessions per current service agreement.',
      'Introduce home practice routine — provide carer coaching handout.',
      'Schedule fortnightly review meeting with Support Coordinator.',
      'Escalate for BSP review given observed pattern changes.',
    ],
  };

  const useLastSessionAsTemplate = () => {
    const lastNote = [...caseNotes]
      .filter((n: CaseNote) => n.clientId === selectedClient && n.status === 'Approved')
      .sort((a: CaseNote, b: CaseNote) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
    if (!lastNote) {
      addNotification({ title: 'No Previous Session', message: 'No approved session notes found for this participant.', type: 'clinical', severity: 'info' });
      return;
    }
    setFormat((lastNote.format as any) || 'SIMPL');
    setSubjective(lastNote.subjective || '');
    setObjective(lastNote.objective || '');
    setAssessment(lastNote.assessment || '');
    setPlan(lastNote.plan || '');
    setSessionDuration(lastNote.sessionDurationMinutes || 60);
    setSelectedGoalIds(lastNote.linkedGoalIds || []);
    addNotification({ title: 'Session Template Loaded', message: `Pre-filled from ${lastNote.date} session for ${selectedClientObj?.name}.`, type: 'clinical', severity: 'info' });
  };
  ...
  const handleSaveNote = (e: React.FormEvent) => {
    ...
    // Persist inline GAS scores entered during this note
    Object.entries(goalGasScores).forEach(([goalId, gasScore]) => {
      if (selectedGoalIds.includes(goalId) && selectedClientObj?.goals) {
        const updatedGoals = selectedClientObj.goals.map((g: ClientGoal) =>
          g.id === goalId
            ? {
                ...g,
                gasScore,
                gasHistory: [...(g.gasHistory || []), { date: new Date().toISOString().slice(0, 10), score: gasScore }],
                progressPercent: Math.max(0, Math.min(100, (gasScore + 2) * 25)),
              }
            : g
        );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <form onSubmit={handleSaveNote} className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4 shadow-sm">
          ...
          {/* Template & Quick Phrases Bar */}
          <div className="flex items-center gap-2 flex-wrap">
            <button type="button" onClick={useLastSessionAsTemplate}
              className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-teal-300 text-xs font-bold rounded-lg border border-teal-500/20 flex items-center gap-1.5 transition-all">
              <Clock className="w-3.5 h-3.5" /> Use Last Session as Template
            </button>
            <button type="button" onClick={() => setShowQuickPhrases(!showQuickPhrases)}
              className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-amber-300 text-xs font-bold rounded-lg border border-amber-500/20 flex items-center gap-1.5 transition-all">
              <Sparkle className="w-3.5 h-3.5" /> {showQuickPhrases ? 'Hide' : 'Quick Phrases'}
            </button>
            {(() => {
              const draftNote = caseNotes.find((n: CaseNote) => n.clientId === selectedClient && n.status === 'Draft');
              return draftNote ? (
                <span className="px-2 py-1 bg-amber-500/10 text-amber-300 text-[10px] font-bold rounded border border-amber-500/20 flex items-center gap-1">
                  <FileCheck className="w-3 h-3" /> Draft exists from {draftNote.date}
                </span>
              ) : null;
            })()}
          </div>
          ...
          {showQuickPhrases && (
            <div className="bg-slate-950/80 rounded-xl border border-amber-500/20 p-3 space-y-2 text-xs">
              <p className="text-[10px] font-bold text-amber-400 uppercase tracking-wider">Quick Clinical Phrases — click to append to field</p>
              {(['subjective', 'objective', 'assessment', 'plan'] as SoapField[]).map(field => (
                <div key={field} className="space-y-1">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">{field}</span>
                  <div className="flex flex-wrap gap-1">
                    {QUICK_PHRASES[field]?.map((phrase, i) => (
                      <button key={i} type="button"
                        onClick={() => {
                          if (field === 'subjective') setSubjective(p => p ? `${p} ${phrase}` : phrase);
                          else if (field === 'objective') setObjective(p => p ? `${p} ${phrase}` : phrase);
                          else if (field === 'assessment') setAssessment(p => p ? `${p} ${phrase}` : phrase);
                          else setPlan(p => p ? `${p} ${phrase}` : phrase);
                        }}
                        className="px-2 py-1 bg-slate-800 hover:bg-amber-500/10 hover:text-amber-300 text-slate-300 rounded-lg border border-slate-700 hover:border-amber-500/30 transition-all text-[10px] text-left">
                        {phrase.substring(0, 55)}…
                      </button>
                    ))}
                  </div>
                </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {isChecked && (
                          <div className="flex items-center gap-1 mt-1">
                            <span className="text-[9px] text-slate-500 font-bold">GAS Score:</span>
                            {([-2, -1, 0, 1, 2] as const).map(score => (
                              <button key={score} type="button"
                                onClick={(e) => { e.preventDefault(); setGoalGasScores(prev => ({ ...prev, [g.id]: score })); }}
                                className={`w-5 h-5 rounded text-[9px] font-black transition-all ${
                                  goalGasScores[g.id] === score
                                    ? score >= 1 ? 'bg-emerald-500 text-white' : score === 0 ? 'bg-amber-500 text-white' : 'bg-rose-500 text-white'
