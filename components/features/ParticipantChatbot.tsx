...
import { runParticipantChatbotQuery, askParticipantChatbotAsync } from '@/lib/participantChatbot';
import { Client, ScheduledShift, ChatbotMessage } from '@/types';
import { useManagementStore } from '@/stores/useManagementStore';

interface ParticipantChatbotProps {
  client?: Client;
...
export const ParticipantChatbot: React.FC<ParticipantChatbotProps> = ({
  client,
  appointments = [],
  isOpen = true,
  onClose,
  ...
  const handleSend = async (queryText?: string) => {
    ...
    try {
      ...
      const result = await askParticipantChatbotAsync(textToSend, {
        client,
        appointments,
        goals: client?.goals || []
      });
      ...
      if (result.isCrisis) {
        addNotification({
          ...
          message: `Participant triggered crisis helpline response in AI chatbot. Escalated to ${result.escalatedTo || 'Clinical Lead'}.`,
          type: 'clinical',
          severity: 'high',
          linkTab: 'incidents'
        });
        ...
        // Non-crisis clinical escalation
        addNotification({
          title: `Clinical Inquiry: ${client?.name || 'Participant'}`,
          message: `Participant asked a complex support question requiring practitioner follow-up: "${textToSend.slice(0, 80)}..."`,
          type: 'clinical',
          severity: 'medium',
          linkTab: 'clients'
        });
        ...
        addAuditLog(
          'PARTICIPANT_CHATBOT_CLINICAL_ESCALATION',
          'PARTICIPANT_PORTAL',
          client?.id || 'anonymous',
          `Complex clinical question routed to practitioner ${result.escalatedTo}.`
        );
      }
      ...
      const botMsg: ChatbotMessage = {
        id: `bot-${Date.now()}`,
        sender: 'assistant',
        text: result.reply || result.message || 'I am here to assist with your plan details.',
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
  ...
  const content = (
    <div className={`bg-slate-900 border border-slate-800 rounded-2xl flex flex-col shadow-2xl overflow-hidden ${
      isModal ? 'w-full max-w-lg h-[600px]' : 'w-full h-[550px]'
    }`}>
      {/* Header */}
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            >
              ...
              {msg.isEscalated && !msg.isCrisis && (
                <div className="flex items-center gap-1.5 font-bold text-amber-300 pb-1 border-b border-amber-500/30 text-[11px]">
                  <CheckCircle2 className="w-3.5 h-3.5 text-amber-400" />
                  <span>Practitioner Escalation Dispatched</span>
                </div>
              )}
