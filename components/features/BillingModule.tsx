...
export const BillingModule: React.FC = () => {
  ...
  const slaAtRisk = billingClaims.filter((c: any) =>
    c.status === 'Submitted PACE' &&
    c.slaDeadline &&
    new Date(c.slaDeadline).getTime() <= Date.now() + 3 * 24 * 60 * 60 * 1000 &&
  ...
  return (
    <div className="space-y-6">
      {/* SLA Breach Watchdog Banner (Phase 5.2) */}
      {!slaAlertDismissed && slaAtRisk.length > 0 && (
        <div className="bg-rose-950/40 border border-rose-500/40 rounded-xl p-3.5 flex items-start gap-3 animate-in fade-in duration-150">
          <AlertTriangle className="w-5 h-5 text-rose-400 shrink-0 mt-0.5" />
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-rose-300">
              SLA Breach Risk: {slaAtRisk.length} Claim{slaAtRisk.length !== 1 ? 's' : ''} Approaching Deadline
            </p>
            <p className="text-xs text-slate-400 mt-0.5 truncate">
              {(slaAtRisk as any[]).map((c: any) => `${c.clientName} — ${c.invoiceNumber} (due ${c.slaDeadline})`).join(' · ')}
            </p>
          </div>
          <button
            onClick={() => setSlaAlertDismissed(true)}
            className="text-slate-500 hover:text-white shrink-0 p-1 rounded hover:bg-rose-900/30 transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
