  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {/* 7. Company Policy & SOP Knowledge Base (RAG Engine) */}
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-3 flex flex-col justify-between md:col-span-2 lg:col-span-3">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-white flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-emerald-400" />
                Company Policy & SOP Knowledge Base (RAG Compliance Engine)
              </span>
              <span className="text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-bold px-2 py-0.5 rounded font-mono flex items-center gap-1">
                <Check className="w-3 h-3" /> /api/compliance/sop-rag
              </span>
            </div>
            <p className="text-[11px] text-slate-400 leading-relaxed">
              Grounds clinical notes, incident reviews, and restrictive practice authorisations against internal company SOPs and NDIS Practice Standards with explicit clickable citations.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 bg-slate-950 p-2.5 rounded border border-slate-800 text-[10px] font-mono">
              <div>
                <span className="text-slate-500 block">SOPs Ingested:</span>
                <span className="text-white font-bold">4 Active Documents</span>
              </div>
              <div>
                <span className="text-slate-500 block">Standards Version:</span>
                <span className="text-teal-400 font-bold">NDIS Commission 2026.2</span>
              </div>
              <div>
                <span className="text-slate-500 block">Case Note Auto-Audit:</span>
                <span className="text-emerald-400 font-bold">Real-Time Verification</span>
              </div>
            </div>
          </div>
          ...
          <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between gap-2 flex-wrap">
            <button
              onClick={async () => {
                try {
                  const res = await fetch('/api/compliance/sop-rag?q=ndis+practice+standards');
                  const data = await res.json();
                  handleTestGateway('Policy SOP RAG', '/api/compliance/sop-rag');
                  addNotification({
                    title: 'SOP Knowledge Base Ping Successful',
                    message: `Verified RAG semantic index (${data.totalDocuments || 4} SOPs loaded).`,
                    type: 'compliance',
                    severity: 'info',
                  });
                } catch {
              ...
              className="py-1.5 px-4 bg-slate-800 hover:bg-slate-700 text-emerald-300 font-bold text-[11px] rounded border border-slate-700 transition-all"
            >
              Test SOP Knowledge Base Index
            </button>
            <span className="text-[10px] text-slate-500 font-mono">
              Automated compliance grounding for Clinical Notes & BSP Plans
            </span>
          </div>
        </div>
